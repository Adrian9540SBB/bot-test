...
var pL = ExpantaNum(1.61622837e-35)
var uni = ExpantaNum(299792458*365.2422*86400*9.3e10)
...
function format(num, precision=document.getElementById("ope")[0], small=false) {
}
function regfor(num, precision=2, small=false) {
...
function get(e) {
    const prefixes = [
        ...
        const prefixes2 = ['', "Mi", "Mc", "Na", "Pc", "Fm", "At", "Zp", "Yc", "Xn", "Ve", "Me", "Du", "Tr", "Te", "Pe", "He", "Hp", "Ot", "En", "Ve", "Is", "Trc", "Tec", "Pec", "Hec", "Hpc", "Otc", "Enc", "Ht", "Dh", "Trh", "Teh", "Peh", "Hxh", "Heh", "Oth", "Enh"]
        e = Math.floor(e/3)-1;
        let index2 = 0;
...
function formval(notation,value,places,placesUnder1000) {
    if (ExpantaNum.lt(ExpantaNum.log10(value), ExpantaNum(Number.MAX_VALUE))) {
        console.log(Math.log10(value))
        if (value instanceof ExpantaNum) {
            console.log("e")
            var matissa = ExpantaNum.div(value, ExpantaNum.pow(10, ExpantaNum.floor(ExpantaNum.log10(value))))
            var power = ExpantaNum(ExpantaNum.floor(ExpantaNum.log10(value)))
            console.log(matissa + power)
        }
        console.log(matissa)
        matissa = ExpantaNum(ExpantaNum.mul(matissa, ExpantaNum.pow(10, power % 3)))
        console.log(matissa + power)
        if (matissa >= 1000) {
            ExpantaNum.div(ExpantaNum(matissa), ExpantaNum(1000));
            console.log(matissa)
            power++;
        }
        if (notation === "Letters") {
            console.log(matissa)
            return ExpantaNum(matissa) + letter(ExpantaNum(power),'abcdefghijklmnopqrstuvwxyz')
        }
            ...
            else return matissa + " " + get(power);
        };
    } else return regfor(value); 

};

shorten = function(money) {
    return formval("Letters",money,2,2)
};